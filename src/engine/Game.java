...
public class Game {
	...
	public static void loadHeroes(InputStream stream) throws IOException {
		availableHeroes = new ArrayList<>();
	...
	public static void endTurn() throws NotEnoughActionsException, InvalidTargetException {
		...
		ArrayList<Zombie> zombiieeees = (ArrayList<Zombie>) zombies.clone();
		for (Zombie zombie : zombiieeees) {
			zombie.attack();
		...
		for (Hero hero : heroes) {
			...
			{
				Point oldTargetLocation = hero.getTarget().getLocation();
	...
	public static void spawnNewZombie() {
		...
		boolean oldVisibility = map[x][y].isVisible();
		map[x][y] = new CharacterCell(z);
		map[x][y].setVisible(oldVisibility);
	}
